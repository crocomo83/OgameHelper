...
public:
    ...
    bool loadMovingUnits(const QString& path);
    bool loadFixUnits(const QString& path);
    ...
    const Unit& getMovingUnit(MovingUnitType type) const;
    const Unit& getFixUnit(FixUnitType type) const;
...
private:
    ...
    std::map<MovingUnitType, Unit> _movingUnits;
    std::map<FixUnitType, Unit> _fixUnits;
};
